...
di eta, per verificare quanto il risultato su alpha dipende dalla scelta eta=sqrt(6).

Valori testati: eta in {sqrt(3), 2, sqrt(6), 3, 2*sqrt(3)}
cioe' x_0 in {2, 1.5, 1, 0.667, 0.5}

[FIX] Convenzione A allineata al main fit: prior alpha in (-0.33, +0.80)
(prima era (-0.80, +0.33), specchio --> esplorava il lato sbagliato).
"""
import numpy as np
...
print("SENSITIVITY ANALYSIS SU eta  (conv. A: alpha > 0)")
print("="*60)
...
def in_prior(th):
    a, Om, H0, ob, s8 = th
    # [FIX] prior allineato all'analisi principale (era -0.80,+0.33)
...
eta_vals = [np.sqrt(3.0), 2.0, np.sqrt(6.0), 3.0, 2.0*np.sqrt(3.0)]
eta_labels = ["sqrt(3)", "2", "sqrt(6)", "3", "2*sqrt(3)"]
...
print("="*75)
print(f"{'eta':>10} {'x_0':>7} {'chi2_min':>10} {'alpha_MAP':>11} "
      f"{'alpha_50%':>11} {'sigma':>7} {'Rhat':>6}")
for r in results:
    print(f"{r['eta_label']:>10} {r['x0']:>7.3f} {r['chi2']:>10.2f} "
          f"{r['map'][0]:>+11.4f} {r['alpha_med']:>+11.4f} "
          f"{r['sigma_alpha']:>6.2f}sig {r['Rhat']:>6.3f}")
print()
print("Interpretazione: se la significativita' di alpha!=0 resta ~2sigma attraverso")
print("la scelta di eta, il risultato principale e' robusto. Se varia da <1sigma a >3sigma,")
print("c'e' forte dipendenza dal prior implicito su eta = scelta canonica giustificata.")
