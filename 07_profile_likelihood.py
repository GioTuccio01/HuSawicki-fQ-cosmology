...
for n_sig, color, label in [(1, 'orange', r'$1\sigma$: $\Delta\chi^2=+1$ from minimum'),
                              (4, 'red',    r'$2\sigma$: $\Delta\chi^2=+4$ from minimum'),
                              (9, 'purple', r'$3\sigma$: $\Delta\chi^2=+9$ from minimum')]:
...
ax.set_title(r'1D profile likelihood: $\chi^2(\alpha)$ minimized over the other 4 parameters', fontsize=11)
...
fig_a.savefig(os.path.join(out_dir, "07_profile_a_chi2.png"), dpi=150, bbox_inches='tight')
plt.close(fig_a)

# === Figure B: best-fit Om, H0, sigma8 vs alpha ===
fig_b, ax_Om = plt.subplots(figsize=(7.5, 5.5))
ax_H0 = ax_Om.twinx()
ax_s8 = ax_Om.twinx()
...
ax_Om.set_title(r'Best-fit $\Omega_m, H_0, \sigma_8$ at each $\alpha$ ($\eta=\sqrt{6}$)', fontsize=11)
...
plt.tight_layout()
fig_b.savefig(os.path.join(out_dir, "07_profile_b_params.png"), dpi=150, bbox_inches='tight')
plt.close(fig_b)
print(f"\n2 plots saved in {out_dir}: 07_profile_a_chi2.png, 07_profile_b_params.png")
