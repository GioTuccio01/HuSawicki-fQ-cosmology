...
Cross-check del risultato α<0 sostituendo il dataset SN con DES-Y5
(Vincenzi+24, Sánchez+24): 1829 SNe di cui ~1765 dopo cut z≥0.01 e
MUERR<5.

Nota: usa solo errori DIAGONALI (STAT-only) perché il file .csv
contiene MUERR_FINAL; la STAT+SYS covariance 1829x1829 non è stata
fornita. Per la versione full-sys, scaricare STAT+SYS.txt.gz da
CobayaSampler/sn_data/DESY5 e adattare il loader.

ATTESO (dalla letteratura DESI+DES-Y5): forte preferenza per w_0>-1,
w_a<0, che tradotto nel nostro modello f(Q) significa α<0 favorito.
"""
import numpy as np
...
from data_loaders import (load_CC, load_BAO_DESI_official, load_CMB_Planck,
                          load_DESY5, load_fsigma8)

print("="*65)
print("ANALISI f(Q) con DES-Y5 SNe (cross-check)")
print("="*65)

CC = load_CC()
BAO_BLOCKS = load_BAO_DESI_official()   # covarianza ufficiale 12x12
CMB_MEAN, CMB_ICOV = load_CMB_Planck()
SN_Z, SN_MU, C_SN_inv, Ainv_MB, Binv_MB = load_DESY5()
N_SN = len(SN_Z)
...
print(f"  CMB:      3 pt")
print(f"  DES-Y5:   {N_SN} SNe (stat-only)")
...
def chi2_fQ(theta, return_parts=False):
    ...
    # SN: distance moduli già calcolati (DES-Y5), marginalizzo offset
