"""
12_MCMC_DESY5.py — Analisi principale con DES-Y5 SNe al posto di Pantheon+.
...
File richiesti in ./data/ o ../data/ o /mnt/user-data/uploads/:
  - DES-Dovekie_HD.csv
  - DESY5_STAT_SYS.npz   (chiave 'cov' = triangolare upper, chiave 'nsn')
Dopo cut MUERR<5: 1743 SNe (il paper riporta ~1765, differenza dovuta a cut diversi).

Stesso setup di 04 ma sostituisce Pantheon+ con DES-Y5.
...
from data_loaders import (load_CC, load_BAO_DESI, load_CMB_Planck,
                          load_fsigma8)
# Loader DES-Y5 con covarianza STAT+SYS piena (NUOVO)
from data_loaders_DESY5_patch import load_DESY5_full

print("="*65)
print("ANALISI f(Q) con DES-Y5 SNe (STAT+SYS, covarianza piena)")
print("Convenzione A: alpha > 0 e' il regime DESI-favorito")
print("="*65)
...
def chi2_fQ(theta, return_parts=False):
    ...
    # SN: distance moduli + cov STAT+SYS piena, M marginalizzato analiticamente
...
print(f"  chi2_LCDM={chi2_L:.2f}  Delta(LCDM-fQ)={chi2_L-chi2_map:+.2f}")
...
chains = []
for ch in range(4):
    rng = np.random.default_rng(6000+ch)
    st = map_fQ + 0.3*init_scale*rng.standard_normal(5)
    if not in_prior(st): st = map_fQ.copy()
    print(f"  Chain {ch+1}/4...", end=" ", flush=True)
    tc = time()
    s, acc = mh_chain(log_prob, st, init_scale, 5000, 15000, rng)
    chains.append(s)
    print(f"acc={acc:.2f}, {time()-tc:.0f}s")
...
# Salvo risultati per confronto globale
np.savez(os.path.join(os.path.dirname(os.path.abspath(__file__)),
                      "..", "plots", "12_DESY5_results.npz"),
         flat=flat, map_fQ=map_fQ, chi2_map=chi2_map, chi2_L=chi2_L,
         Rhat=Rhat, n_sn=N_SN)
print(f"\nRisultati salvati in ../plots/12_DESY5_results.npz")
print(f"  (incl. n_sn={N_SN}, da usare nel summary)")


# =====================================================================
# CORNER PLOT (stessa estetica di 04)
# =====================================================================
import matplotlib.pyplot as plt

def corner_plot(samples, labels, truths=None, bins=32, color='crimson', cmap='Reds'):
    n = samples.shape[1]
        ...
        for j in range(n):
            ...
            if i == j:
                ...
                ax.set_title(f'{labels[i]}\n'
                    ...
                    fontsize=9)
                if truths is not None: ax.axvline(truths[i], color='navy', lw=1.3)
                ...
                if truths is not None:
                    ax.plot(truths[j], truths[i], 'x', color='navy', ms=7, mew=1.6)
...
fig.suptitle(rf'MCMC with DES-Y5 (STAT+SYS), $\eta = \sqrt{{6}}$ fixed — {N_SN} SNe',
             fontsize=11, y=1.005)

out_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "plots")
os.makedirs(out_dir, exist_ok=True)
out_path = os.path.join(out_dir, "12_DESY5_corner.png")
fig.savefig(out_path, dpi=150, bbox_inches='tight')
print(f"Corner plot salvato: {out_path}")
