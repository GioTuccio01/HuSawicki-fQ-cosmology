...
# Plot
# -----------------------------------------------------------------------------
x_vals = np.linspace(1, 15, 3000)  # da ~0.03 a ~31, copre entrambe le radici
...
ax1.axvline(x0, ls='-', color='green', alpha=0.35, label='oggi')
...
# --- pannello 2: 1 + w_DE zoom sugli attraversamenti ---
for alpha, col in zip(alphas, colors):
    Lam = Lam_from_alpha(alpha)
    w   = w_DE(x_vals, alpha, Lam)
...
plt.tight_layout()
outpath = os.path.join(_OUT_DIR, '02_wDE_phantom.png')
