...
# --------- PLOT: 2 pannelli (2D + 3D) ---------
fig = plt.figure(figsize=(14, 6))

# Pannello A: colormap + isolivelli
ax1 = fig.add_subplot(1, 2, 1)
AG, OG = np.meshgrid(alpha_grid, Om_grid, indexing='ij')

# Cap il colorplot a 3σ per leggibilità
im = ax1.pcolormesh(AG, OG, np.clip(dchi2, 0, 15),
                    cmap='viridis_r', shading='auto', alpha=0.9)
cb = fig.colorbar(im, ax=ax1, label=r'$\Delta\chi^2 = \chi^2_{\rm prof} - \chi^2_{\min}$')
...
ax2.set_title(r'Superficie 3D di $\chi^2_{\rm prof}(\alpha, \Omega_m)$', fontsize=11)
ax2.view_init(elev=25, azim=-65)

plt.tight_layout()
out = os.path.join(OUT_DIR, "07b_profile_2D.png")
