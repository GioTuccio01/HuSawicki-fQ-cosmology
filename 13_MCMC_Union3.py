...
from data_loaders import (load_CC, load_BAO_DESI, load_CMB_Planck,
                          load_fsigma8)
# Loader Union3 con covarianza STAT+SYS piena (NUOVO)
from data_loaders_Union3_patch import load_Union3

print("="*65)
print("ANALISI f(Q) con Union3 SNe (STAT+SYS, covarianza piena)")
...
np.savez(os.path.join(os.path.dirname(os.path.abspath(__file__)),
         ...
         Rhat=Rhat, n_sn=N_SN)
print(f"\nRisultati salvati in ../plots/13_Union3_results.npz")
...
def corner_plot(samples, labels, truths=None, bins=32, color='darkorange', cmap='Oranges'):
    n = samples.shape[1]
        ...
        for j in range(n):
            ...
            if i == j:
                ax.hist(samples[:, i], bins=bins, color=color, alpha=0.85,
                        edgecolor='saddlebrown', lw=0.3, density=True)
                ...
                try:
                    ...
                    if len(levs) >= 1:
                        ...
                        ax.contour(xc, yc, H.T, levels=levs, colors='saddlebrown', linewidths=0.9)
                except Exception:
                    pass
...
fig.suptitle(rf'MCMC with Union3 (binned), $\eta = \sqrt{{6}}$ fixed — {N_SN} bins',
             fontsize=11, y=1.005)

out_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "plots")
os.makedirs(out_dir, exist_ok=True)
out_path = os.path.join(out_dir, "13_Union3_corner.png")
