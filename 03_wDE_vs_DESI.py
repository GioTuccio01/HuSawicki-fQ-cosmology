...
# CPL DESI come guida visiva (non fit del mio modello, sono parametri DESI DR1)
...
for et, c in zip(etas, cols_b):
    ...
    # marca il phantom crossing (z dove x = x_+)
...
ax2.set(xlabel=r'$z$', ylabel=r'$w_{\rm DE}(z)$',
        title=fr'(b)  $w_{{\rm DE}}(z)$  a $\alpha={alpha_b:+.2f}$  (vari $\eta=\mu/H_0$)')
ax2.grid(alpha=0.3)
ax2.legend(fontsize=8.5, loc='lower left', framealpha=0.92)
ax2.set_xlim(0, 2); ax2.set_ylim(-1.20, -0.78)

plt.tight_layout()
out = os.path.join(_OUT_DIR, '03_wDE_vs_DESI.png')
