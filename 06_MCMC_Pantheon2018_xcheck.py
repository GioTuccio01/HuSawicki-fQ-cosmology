"""
06_MCMC_Pantheon2018_xcheck.py — Cross-check con Pantheon 2018.

Stesso setup di 04 ma sostituisce Pantheon+ (1580 SN) con Pantheon 2018.
Serve per verificare la robustezza del segnale alpha cambiando il dataset SN.

Due modalita':
  UNBINNED = False: Pantheon 2018 binned (40 pt, default del repo).
  UNBINNED = True : Pantheon 2018 unbinned (1048 SNe individuali).
                    Richiede file lcparam_full_long.txt e sys_full_long.txt
                    in /tmp/TESI_fQ_cosmology/data/. Scaricarli da
                    https://github.com/dscolnic/Pantheon (vedi loader).

Risultati attesi:
  binned   : alpha = +0.013 +/- 0.085   (0.15 sigma, negligible)
  unbinned : atteso valore simile (il binning di Scolnic preserva
             l'informazione cosmologica al ~3%).
"""
import numpy as np
...
from data_loaders import (load_CC, load_BAO_DESI, load_CMB_Planck,
                          load_Pantheon2018_bin, load_Pantheon2018_unbinned,
                          load_fsigma8)

# --------------------------------------------------
# SCELTA DATASET: cambia solo questa riga
UNBINNED = True    # True = 1048 SNe; False = 40 bin
# --------------------------------------------------
...
if UNBINNED:
    print("  [Pantheon 2018 UNBINNED: 1048 SNe individuali]")
    try:
        SN_Z, SN_MB, C_SN_inv, Ainv_MB, Binv_MB = load_Pantheon2018_unbinned()
    except FileNotFoundError as e:
        print(f"\nERRORE: {e}")
        print("\nFallback a binned (40 pt)\n")
        UNBINNED = False
        SN_Z, SN_MB, C_SN_inv, Ainv_MB, Binv_MB = load_Pantheon2018_bin()
else:
    print("  [Pantheon 2018 BINNED: 40 pt]")
    SN_Z, SN_MB, C_SN_inv, Ainv_MB, Binv_MB = load_Pantheon2018_bin()
N_SN = len(SN_Z)
FS8 = load_fsigma8(); FS8_Z, FS8_V, FS8_S = FS8[:,0], FS8[:,1], FS8[:,2]
N_TOT = len(CC) + sum(len(b['kinds']) for b in BAO_BLOCKS) + 3 + N_SN + len(FS8)
SN_TAG = "unbinned" if UNBINNED else "binned"
print(f"  {len(CC)} CC, 12 BAO, 3 CMB, {N_SN} SN ({SN_TAG}), {len(FS8)} fsigma8 = {N_TOT} pt\n")
...
def mh_chain(log_p, start, init_scale, n_burn, n_prod, rng):
    ...
    for i in range(n_prod):
        prop=cur+chol@rng.standard_normal(ndim); plp=log_p(prop)
        if plp-cur_lp > np.log(rng.random()+1e-300):
            cur,cur_lp=prop,plp; acc+=1
        prod[i]=cur
...
AIC_fQ=chi2_map+2*5; AIC_L=resL.fun+2*4
BIC_fQ=chi2_map+5*np.log(N_TOT); BIC_L=resL.fun+4*np.log(N_TOT)
print(f"\nDelta AIC (LCDM-fQ) = {AIC_L-AIC_fQ:+.2f}")
print(f"Delta BIC (LCDM-fQ) = {BIC_L-BIC_fQ:+.2f}")
...
def corner_plot(samples, labels, truths=None, bins=32):
    ...
    for i in range(n):
        for j in range(n):
            ...
            if i==j:
                ax.hist(samples[:,i], bins=bins, color='seagreen', alpha=0.85,
                        edgecolor='darkgreen', lw=0.3, density=True)
                qq = np.quantile(samples[:,i],[0.16,0.5,0.84])
                for q_,s_ in zip(qq,[':','-',':']):
                    ax.axvline(q_, color='k', lw=1, ls=s_)
                ax.set_title(f'{labels[i]}\n'
                    fr'${qq[1]:.4g}^{{+{qq[2]-qq[1]:.2g}}}_{{-{qq[1]-qq[0]:.2g}}}$', fontsize=9)
                ...
                ax.pcolormesh(xe, ye, H.T, cmap='Greens', shading='auto')
                ...
                try:
                    ...
                    if len(levs)>=1:
                        ...
                        ax.contour(xc, yc, H.T, levels=levs, colors='darkgreen', linewidths=0.9)
                except: pass
...
fig.suptitle(r'Cross-check: Pantheon 2018 binned (40 pt)', fontsize=11, y=1.005)
out_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "plots")
os.makedirs(out_dir, exist_ok=True)
fig.savefig(os.path.join(out_dir, "06_Pan2018_corner.png"), dpi=150, bbox_inches='tight')
print(f"\nCorner salvato in {out_dir}")
